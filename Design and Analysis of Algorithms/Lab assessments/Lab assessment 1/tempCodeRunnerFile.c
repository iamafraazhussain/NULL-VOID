...
void main()
{
    ...
    printf("Enter the city you would like to start from: ");
    scanf("%d", &origin);
    origin--;
    ...
    for (row = 1; row <= numberOfCities - 1; row++)
    {
        for (column = 0; column < numberOfCities - 2; column++)
        {
            ...
            if (leastDistance == -1.0)
            {
                ...
                for (index = 1; index < numberOfCities - 1; index++) bestRoute[index] = numberCombinations[index - 1];
            }
    ...
    printf("\nTo achieve this, you'll have to use the following route: %d", origin + 1);
